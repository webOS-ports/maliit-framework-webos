...
int MInputMethodHost::enterKeyType(bool &valid)
{
    return connection->enterKeyType(valid);
}
...
bool MInputMethodHost::maxTextLength(int &maxTextLength)
{
    return connection->maxTextLength(maxTextLength);
}

bool MInputMethodHost::platformData(QString &pattern)
{
    return connection->platformData(pattern);
}
...
{
    if (enabled) {
        webOSLogInfo("SETLANGUAGE", "STATE_CHANGE", language);
